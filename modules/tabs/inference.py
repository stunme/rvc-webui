...
def inference_options_ui(show_out_dir=True):
    ...
    # ===== 包装 source_audio：上传文件存在则覆盖文本框 =====
    # 返回一个 gr.Variable 风格的引用（点击按钮时会自动取值）
    def get_source_audio():
        if source_audio_file.value:
            # Gradio 返回的 file 是 dict
            if isinstance(source_audio_file.value, dict) and "name" in source_audio_file.value:
                return source_audio_file.value["name"]
            return source_audio_file.value
        return source_audio_text.value
